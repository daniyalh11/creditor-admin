...
export const ConfigureExportModal = ({
  isOpen,
  onClose,
  currentConfig,
  onSave
  ...
  const handleFieldToggle = (category, field, checked) => {
    setConfig(prev => ({
      ...prev,
      selectedFields: {
        ...prev.selectedFields,
                ...
                <div className="grid grid-cols-2 gap-2">
                  {fields.map((field) => (
                    <div key={field} className="flex items-center space-x-2">
                      <Checkbox
                        id={`${category}-${field}`}
                        checked={config.selectedFields[category].includes(field)}
                        onCheckedChange={(checked) => handleFieldToggle(category, field, checked)}
                      />
