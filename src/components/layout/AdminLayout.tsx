...
const getAdminContext = (pathname: string) => {
  if (pathname.includes('/admin/accounts')) {
    ...
    return {
      tabs: [], // AutomationSettings.tsx handles its own tabs
      activeTab: "" 
    };
  } else if (pathname.startsWith('/admin/moderate')) {
    return {
      tabs: [], // ModerateSettings.tsx will handle its own pill-style tabs
      activeTab: ""
    };
  } else if (pathname.startsWith('/admin/policies')) { // Added this condition
    return {
      tabs: [], // PoliciesSettings.tsx will handle its own pill-style tabs
      activeTab: ""
    };
  }
...
export const AdminLayout = ({ title }: AdminLayoutProps) => {
  ...
  const { 
    ...
    // setAdminSectionActive, // Now primarily driven by context/navigation
    isAdminSidebarOpen,
    toggleAdminSidebar,
    closeAdminPanel,
    openAdminPanel, // Added for consistency

    isGroupSectionActive,
    // setGroupSectionActive, // Now primarily driven by context/navigation
    isGroupSidebarOpen,
    toggleGroupSidebar,
    closeGroupPanel,
    openGroupPanel, // Added
    activeGroupId // Get activeGroupId from context
  ...
  const handleToggleAdminSidebar = () => {
    if (isAdminSidebarOpen) {
      closeAdminPanel();
      if (!isGroupSectionActive) navigate('/'); // Navigate away if not also in group section
    } else {
      openAdminPanel();
      // navigate('/admin'); // Let AdminNavigation handle default admin page
    }
    toast({
  ...
  const handleToggleGroupSidebar = () => {
    if (isGroupSidebarOpen) {
      closeGroupPanel();
      // navigate('/groups'); // Navigate to groups list when closing
    } else {
      if (activeGroupId) { // Ensure there's a group ID to open for
        openGroupPanel(activeGroupId);
      }
      // Consider navigation or checks here
    }
     toast({
  ...
  useEffect(() => {
    if (!isAdminSectionActive && isAdminSidebarOpen) {
      // closeAdminPanel(); // This could cause navigation loops if not careful
    }
  ...
  useEffect(() => {
    ...
    // No explicit navigation for group default here, App.tsx will handle it.
    
    // setAdminSectionActive(currentPath.startsWith('/admin')); // Context handles this
    // setGroupSectionActive(currentPath.startsWith('/groups/view/')); // Context handles this
  ...
  // Determine which contextual sidebar is active (simplified logic, assumes only one can be truly "active" for layout)
  ...
  // Width of the contextual sidebar (admin or group)
  const contextualSidebarWidth = "16rem"; // Assuming fixed width for now
  // Whether the contextual sidebar should appear collapsed (e.g. for icon-only view)
  // For now, let's assume contextual sidebars are not collapsible themselves in this manner, but main can be.
  const isContextualSidebarVisuallyCollapsed = false; // Placeholder for future feature

  // Check if we should show floating back button (for embedded content in admin)
  const shouldShowFloatingBackButton = location.pathname.startsWith('/admin') && 
    !location.pathname.endsWith('/admin') && 
    !tabs.length; // Don't show on pages with tabs
  ...
  return (
    <div className="min-h-screen bg-background flex">
      ...
      {(showAdminContextualSidebar || showGroupContextualSidebar) && (
        <div
          className={cn(
            ...
            // Mobile handling: slides in from left, or is fixed based on screen size
            isMobile ? (isContextualSidebarOpen ? 'translate-x-0 left-0 w-64' : '-translate-x-full w-64') 
                     : '', // Desktop positioning below
            "animate-fade-in"
          )}
          style={{ 
        ...
        >
          ...
          {showGroupContextualSidebar && (
             // GroupContextualSidebar now manages its own header and scrolling
      ...
      {/* Contextual Sidebar Toggle Button (Example for Admin, adapt for Group or generic toggle) */}
      {/* This button's position and logic need to be smart based on which sidebar is potentially openable */}
      {!isMobile && (
        <>
          ...
          {isGroupSectionActive && !isGroupSidebarOpen && activeGroupId && (
             <div 
              className="fixed z-50 top-5 transition-all duration-500 animate-pulse"
              // Position this button correctly if admin sidebar is NOT the one to open
              style={{ left: isMainCollapsed ? '4.5rem' : '16.5rem' }} 
            >
              <Button
      ...
      {/* Floating Back Button - Only show on admin pages without tabs */}
