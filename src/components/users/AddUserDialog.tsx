...
export function AddUserDialog({ open, onOpenChange }: AddUserDialogProps) {
  ...
  const singleForm = useForm<AddUserFormData>({
    defaultValues: {
      role: selectedRole === 'all' ? 'learner' : selectedRole,
    }
  });

  const bulkForm = useForm<BulkUserFormData>({
    defaultValues: {
      users: [
  ...
  const onSingleSubmit = (data: AddUserFormData) => {
    const newUser = {
      name: data.name,
      email: data.email,
      role: [data.role],
  ...
  const onBulkSubmit = (data: BulkUserFormData) => {
