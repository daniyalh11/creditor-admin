...
/**
 * @typedef {'skill' | 'competency' | 'objective'} MasteryType
 * @typedef {'Beginner' | 'Intermediate' | 'Advanced'} MasteryLevel
 */
 ...
 * @property {MasteryType} type
 * @property {MasteryLevel} level
 * @property {number} requiredScore
 */

/**
 * A modal for adding a new mastery item (skill, competency, or objective).
 *
 * @param {object} props
 * @param {boolean} props.open - Controls if the modal is open.
 * @param {(open: boolean) => void} props.onOpenChange - Callback to change the open state.
 * @param {(masteryData: MasteryData) => void} props.onAdd - Callback to add the new mastery item.
 */
export const AddMasteryModal = ({
  open,
  onOpenChange,
  onAdd
}) => {
          ...
          <form onSubmit={handleSubmit} className="p-4 md:p-6 space-y-6">
            ...
            <div className="space-y-2">
              ...
              <Input
                ...
                onChange={(e) => setFormData({ ...formData, requiredScore: parseInt(e.target.value, 10) || 80 })}
              />
