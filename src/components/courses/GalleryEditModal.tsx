...
export const GalleryEditModal: React.FC<GalleryEditModalProps> = ({
  ...
  const [style, setStyle] = useState<'carousel' | '2-column' | '3-column' | '4-column'>('carousel');
  ...
  const updateImage = (id: string, field: 'src' | 'caption', value: string) => {
  ...
  const handleImageUpload = (id: string, event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file) {
      ...
      reader.onload = (e) => {
        updateImage(id, 'src', e.target?.result as string);
      };
      ...
      <DialogContent className="w-full max-w-4xl max-h-[90vh] mx-4 flex flex-col">
        ...
        <div className="flex-1 overflow-y-auto space-y-4 py-4">
          <div>
            ...
            <Select value={style} onValueChange={(value: 'carousel' | '2-column' | '3-column' | '4-column') => setStyle(value)}>
              <SelectTrigger>
                <SelectValue />
              </SelectTrigger>
              <SelectContent>
