...
import { Plus, X, Upload, Grid3X3, Layers, Image } from 'lucide-react';
...
export const FlashcardEditModal: React.FC<FlashcardEditModalProps> = ({
  ...
  const frontImageInputRefs = useRef<{ [key: string]: HTMLInputElement | null }>({});
  const backImageInputRefs = useRef<{ [key: string]: HTMLInputElement | null }>({});
  ...
  const handleUpdateCard = (cardId: string, side: 'front' | 'back', field: 'text' | 'image', value: string) => {
  ...
  const handleImageUpload = (cardId: string, side: 'front' | 'back', file: File) => {
  ...
  const handleImageButtonClick = (cardId: string, side: 'front' | 'back') => {
    const inputRef = side === 'front' ? frontImageInputRefs.current[cardId] : backImageInputRefs.current[cardId];
    inputRef?.click();
  };

  const handleRemoveImage = (cardId: string, side: 'front' | 'back') => {
