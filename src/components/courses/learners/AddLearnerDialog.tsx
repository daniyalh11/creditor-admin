...
interface AddLearnerDialogProps {
  isOpen: boolean;
  onClose: () => void;
  onAdd: (learnerData: { userId: string; name: string; email: string; role?: string }) => void;
}

export const AddLearnerDialog: React.FC<AddLearnerDialogProps> = ({
  isOpen,
  onClose,
  onAdd
}) => {
                ...
                <div className="flex items-center justify-between">
                  ...
                  <div className="flex items-center gap-2">
                    <Checkbox
                      ...
                      id="select-all-users"
                    />
                    <label htmlFor="select-all-users" className="text-sm text-gray-600 cursor-pointer">
                  ...
                  <div className="p-3 border rounded-lg bg-green-50">
                    <div className="flex flex-wrap gap-2">
                      {selectedUsers.slice(0, 5).map((user) => (
                        <Badge key={user.id} variant="secondary" className="bg-white">
                          {user.name}
                        </Badge>
                      ))}
