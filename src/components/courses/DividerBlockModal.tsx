...
interface DividerBlockModalProps {
  ...
  onSelect: (dividerType: 'continue' | 'divider' | 'number' | 'space') => void;
}

export const DividerBlockModal: React.FC<DividerBlockModalProps> = ({
  open,
  onOpenChange,
  onSelect
}) => {
  const handleSelect = (type: 'continue' | 'divider' | 'number' | 'space') => {
    onSelect(type);
    onOpenChange(false);
  };
