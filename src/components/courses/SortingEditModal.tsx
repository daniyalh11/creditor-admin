...
import { Trash2, Plus, GripVertical, Upload, X, Image } from 'lucide-react';
...
interface SortingEditModalProps {
  open: boolean;
  onOpenChange: (open: boolean) => void;
  block: SortingBlock;
  onSave: (block: SortingBlock) => void;
}

export const SortingEditModal: React.FC<SortingEditModalProps> = ({
  open,
  onOpenChange,
  block,
  onSave
  ...
  useEffect(() => {
    if (block && block.style === 'sorting') {
      const content = block.content as SortingContent;
  ...
  const handleItemImageChange = (itemId: string, newImage: string) => {
    setItems(items.map(item => 
      item.id === itemId ? { ...item, image: newImage || undefined } : item
    ));
  };
  ...
  const handleSave = () => {
    ...
    const updatedBlock: SortingBlock = {
      ...block,
      content: {
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-5xl max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle>Edit Sorting Activity</DialogTitle>
        </DialogHeader>
        ...
        <div className="space-y-6">
          ...
          <div className="space-y-4">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                <Label htmlFor="newItemText">Card Text</Label>
                <Input
                  id="newItemText"
                  value={newItemText}
              ...
              <div>
                <Label htmlFor="newItemImage">Card Image URL (Optional)</Label>
                <Input
                  id="newItemImage"
                  value={newItemImage}
              ...
              <div className="flex gap-2 mt-2">
                {sampleImages.map((imageUrl, index) => (
                  <button
                    key={index}
                    type="button"
                    onClick={() => setNewItemImage(imageUrl)}
            ...
            {/* Preview New Card */}
            {(newItemText || newItemImage) && (
              <div className="border rounded-lg p-3 bg-gray-50">
                <Label className="text-sm text-gray-600 mb-2 block">Preview:</Label>
                <div className="bg-white border rounded-lg p-3 w-fit min-w-[200px]">
                  <div className="flex flex-col items-center space-y-2">
                    ...
                    {newItemText && (
                      <span className="text-sm font-medium text-center">{newItemText}</span>
                    )}
                  </div>
                </div>
              </div>
              ...
              {items.map((item, index) => (
                <Card key={item.id} className="border-2">
                  <CardContent className="p-4">
                    <div className="space-y-4">
                      {/* Header with position and controls */}
          ...
          <div className="space-y-4">
            ...
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-sm text-gray-600">Allow learners to try again after incorrect attempts</p>
              </div>
              <Switch
                id="allowRetry"
                checked={allowRetry}
            ...
            <div className="flex items-center justify-between">
              <div>
                <Label htmlFor="showCorrectOrder">Show Correct Order</Label>
                <p className="text-sm text-gray-600">Show correct order after incorrect attempts</p>
              </div>
              <Switch
                id="showCorrectOrder"
                checked={showCorrectOrder}
                onCheckedChange={setShowCorrectOrder}
              />
            </div>
          </div>
