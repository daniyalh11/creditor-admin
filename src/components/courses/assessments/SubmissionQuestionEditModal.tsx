...
interface SubmissionQuestion {
  ...
  type: 'submission';
  question: string;
  submissionType?: 'drive-link' | 'text-submission' | 'file-upload';
  driveLink?: string;
  notes?: string;
}

interface SubmissionQuestionEditModalProps {
  question: SubmissionQuestion;
  onSave: (question: SubmissionQuestion) => void;
  onCancel: () => void;
}

export const SubmissionQuestionEditModal: React.FC<SubmissionQuestionEditModalProps> = ({
  question,
  onSave,
  onCancel,
}) => {
  const [editedQuestion, setEditedQuestion] = useState<SubmissionQuestion>({
    ...question,
  ...
  const handleSubmissionTypeChange = (submissionType: 'drive-link' | 'text-submission' | 'file-upload') => {
    setEditedQuestion({
      ...editedQuestion,
      submissionType,
  ...
  const handleDriveLinkChange = (driveLink: string) => {
    setEditedQuestion({
      ...editedQuestion,
      driveLink,
    });
        ...
        <div className="space-y-6">
          ...
          <div className="space-y-3">
            ...
            <RadioGroup
              ...
              onValueChange={(value) => handleSubmissionTypeChange(value as 'drive-link' | 'text-submission' | 'file-upload')}
            >
