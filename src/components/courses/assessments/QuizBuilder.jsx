...
export const QuizBuilder = ({
  ...
  useEffect(() => {
    const handleAddMCQQuestion = (event) => {
    ...
    const handleAddDescriptiveQuestion = (event) => {
      const isSurvey = event.detail?.isSurvey || false;
      const newQuestion = {
        id: Date.now().toString(),
        type: 'descriptive',
          ...
          <div className="space-y-2">
            ...
            {question.options.map((option, idx) => (
              <div key={idx} className="flex items-center space-x-2">
                <input type={question.type === 'mcq' && question.options.filter(o => o.isCorrect).length > 1 ? "checkbox" : "radio"} disabled className="text-blue-600" />
