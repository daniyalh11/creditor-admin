...
const CourseCard = ({ id, title, description, courseCount, imageUrl }) => {
  ...
  const handleCardClick = () => {
    ...
    // Using a more generic path that uses the ID is often more robust than using the title
    navigate(`/courses/${id}`); 
    // Or, to keep the exact original behavior:
...
CourseCard.propTypes = {
  ...
  imageUrl: PropTypes.string, // imageUrl is optional, so it's not marked as isRequired
};
