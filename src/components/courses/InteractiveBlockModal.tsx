...
interface InteractiveBlockModalProps {
  open: boolean;
  onOpenChange: (open: boolean) => void;
  onSelect: (interactiveType: 'accordion' | 'tabs' | 'labeled-graphic' | 'process' | 'flashcard' | 'timeline' | 'sorting' | 'scenario') => void;
}

export const InteractiveBlockModal: React.FC<InteractiveBlockModalProps> = ({
  open,
  onOpenChange,
  onSelect
}) => {
  const handleSelect = (type: 'accordion' | 'tabs' | 'labeled-graphic' | 'process' | 'flashcard' | 'timeline' | 'sorting' | 'scenario') => {
    onSelect(type);
    onOpenChange(false);
  };
  ...
  const interactiveTypes = [
    ...
    {
      type: 'labeled-graphic' as const,
      icon: Tag,
      title: 'Labeled Graphic',
      description: 'Interactive image with hotspots',
      preview: (
        <div className="w-full bg-gray-100 rounded p-4 relative">
    ...
    },
    {
      ...
      preview: (
        <div className="w-full bg-blue-50 border-2 border-blue-200 rounded p-4 text-center">
    ...
    },
    {
      ...
      preview: (
        <div className="w-full bg-green-50 border border-green-200 rounded p-3 text-center">
