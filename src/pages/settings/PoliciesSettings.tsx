...
const PoliciesSettings = () => {
  ...
  const [historyModal, setHistoryModal] = useState<{isOpen: boolean, policyKey: string, policyTitle: string}>({
    isOpen: false,
    policyKey: '',
    policyTitle: ''
  });
  ...
  const handleSavePolicy = (policyKey: string) => {
    toast({
  ...
  const handleTogglePolicy = (policyKey: string) => {
    setPolicies(prev => ({
      ...prev,
      [policyKey]: {
        ...prev[policyKey as keyof typeof prev],
        enabled: !prev[policyKey as keyof typeof prev].enabled
      }
    }));
    toast({
      title: "Policy Status Updated",
      description: `${policyKey} has been ${policies[policyKey as keyof typeof policies].enabled ? 'disabled' : 'enabled'}.`
    });
  };

  const handleViewHistory = (policyKey: string, policyTitle: string) => {
    setHistoryModal({
      isOpen: true,
      policyKey,
      policyTitle
  ...
  const handleRestoreVersion = (version: string) => {
    ...
    const currentPolicy = historyModal.policyKey as keyof typeof policies;
    setPolicies(prev => ({
      ...prev,
      [currentPolicy]: {
        ...prev[currentPolicy],
          ...
          <Card key={policy.key}>
                ...
                <div className="flex items-center gap-4">
                  <div className="flex items-center gap-2">
                    ...
                    <Switch
                      ...
                      checked={policies[policy.key as keyof typeof policies].enabled}
            ...
            <CardContent className="space-y-4">
              <div className="flex items-center gap-4 text-sm text-muted-foreground">
                <span>Version: {policies[policy.key as keyof typeof policies].version}</span>
                <span>Last Updated: {policies[policy.key as keyof typeof policies].lastUpdated}</span>
              </div>
              
              {editingPolicy === policy.key ? (
                <div className="space-y-4">
                  <Textarea
                    value={policies[policy.key as keyof typeof policies].content}
                    onChange={(e) => setPolicies(prev => ({
                      ...prev,
                      [policy.key]: {
                        ...prev[policy.key as keyof typeof prev],
                ...
                <div className="space-y-4">
                  <div className="p-4 bg-gray-50 rounded-lg">
                    <p className="text-sm">{policies[policy.key as keyof typeof policies].content}</p>
                  </div>
                  <div className="flex gap-2">
