...
const CatalogSettings = () => {
  ...
  const handleOptionChange = (optionSetId: string, optionId: string, checked: boolean) => {
  ...
  const handleSaveIntroduction = (newIntroduction: string) => {
  ...
  const handleSaveBox = (boxData: Omit<Box, 'id' | 'dateAdded'>) => {
    const newBox: Box = {
  ...
  const handleVisibilityFilter = (categoryId: number) => {
  ...
  const handleSaveVisibilityFilter = (filter: string) => {
  ...
  const handleSEOMetadata = (categoryId: number) => {
    const category = categories.find(c => c.id === categoryId);
    if (category) {
      setSelectedCategory(category);
      setSelectedCategoryId(categoryId);
  ...
  const handleSaveSEOMetadata = (metadata: SEOMetadata) => {
    toast({
  ...
  const handlePictureUpload = (categoryId: number) => {
                    ...
                    <tr key={option.id} className="h-10">
                      ...
                      <td className="text-right">
                        <Checkbox 
                          ...
                          onCheckedChange={(checked) => handleOptionChange('catalog', option.id, checked as boolean)}
                    ...
                    <tr key={option.id} className="h-10">
                      ...
                      <td className="text-right">
                        <Checkbox 
                          id={option.id} 
                          checked={option.checked} 
                          onCheckedChange={(checked) => handleOptionChange('advanced', option.id, checked as boolean)}
                          className="cursor-pointer"
                        />
                      </td>
                    </tr>
                    ...
                    <tr key={option.id} className="h-10">
                      ...
                      <td className="text-right">
                        <Checkbox 
                          id={option.id} 
                          checked={option.checked} 
                          onCheckedChange={(checked) => handleOptionChange('rightColumn', option.id, checked as boolean)}
                          className="cursor-pointer"
                        />
                      </td>
                    </tr>
