...
interface SurveyMCQQuestion extends SurveyQuestion {
  type: 'survey-mcq';
  options: Array<{ id: string; text: string; }>;
}
...
const SurveyBuilder = () => {
  ...
  const [editingQuestion, setEditingQuestion] = useState<SurveyMCQQuestion | null>(null);
  ...
  useEffect(() => {
    if (editId) {
      ...
      const editingAssessment = existingAssessments.find((a: any) => a.id === editId);
      ...
      // Mock loading survey data for new survey
      console.log('Loading survey:', surveyId);
    }
  }, [surveyId, editId]);

  const handleSurveyUpdate = (field: keyof SurveyData, value: any) => {
    setSurveyData(prev => ({
      ...prev,
      [field]: value
    }));
  ...
  const handleAddInstruction = () => {
    const newInstruction: SurveyInstruction = {
  ...
  const handleInstructionUpdate = (instructionId: string, content: string) => {
    setSurveyData(prev => ({
      ...prev,
  ...
  const handleAddQuestion = (type: 'survey-mcq' | 'description') => {
    ...
    if (type === 'survey-mcq') {
      setEditingQuestion(newQuestion as SurveyMCQQuestion);
    }
  };

  const handleQuestionUpdate = (updatedQuestion: SurveyMCQQuestion) => {
    setSurveyData(prev => ({
      ...prev,
  ...
  const handleEditQuestion = (question: SurveyQuestion) => {
    if (question.type === 'survey-mcq') {
      setEditingQuestion(question as SurveyMCQQuestion);
    }
  };
