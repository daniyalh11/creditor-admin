...
const UserGuide = () => {
  ...
  const getFilteredGuides = (tabId: string) => {
  ...
  const handleDuplicate = (guide: any) => {
  ...
  const handleEditSave = (updatedGuide: any) => {
  ...
  const handleDeleteConfirm = (guideId: number) => {
  ...
  const handleContactSupport = (data: { name: string; email: string; message: string }) => {
    toast({
      title: "Message Sent",
  ...
  const renderGuideCards = (guides: any[]) => {
    if (guides.length === 0) {
      return (
