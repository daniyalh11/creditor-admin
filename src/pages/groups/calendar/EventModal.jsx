  ...
  return (
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="max-w-3xl p-6">
        ...
        <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
          ...
          <TabsContent value="summary" className="space-y-4 pt-4">
            <div className="space-y-2">
              ...
              <Input id="event-name" placeholder="Event name" value={eventTitle} onChange={(e) => setEventTitle(e.target.value)} disabled={mode === 'view'} />
            </div>

            <div className="space-y-2">
              <Label>Duration:</Label>
              <div className="grid grid-cols-2 gap-2 items-center">
                <Input type="date" value={eventStartDate ? format(eventStartDate, 'yyyy-MM-dd') : ''} onChange={(e) => setEventStartDate(e.target.value ? new Date(e.target.value) : null)} disabled={mode === 'view'} />
                <span className="text-center">to</span>
                <Input type="date" value={eventEndDate ? format(eventEndDate, 'yyyy-MM-dd') : ''} onChange={(e) => setEventEndDate(e.target.value ? new Date(e.target.value) : null)} disabled={mode === 'view'} />
              </div>
            </div>
          </TabsContent>

          <TabsContent value="description" className="space-y-4 pt-4">
            <SimpleRichTextEditor value={eventDescription} onChange={setEventDescription} placeholder="Add description for your event..." disabled={mode === 'view'} />
          </TabsContent>

          <TabsContent value="repeat" className="pt-4">
            <div className="space-y-6">
              <div className="flex items-center">
                <Checkbox id="repeat-event" checked={repeatEvent} onCheckedChange={(checked) => setRepeatEvent(!!checked)} disabled={mode === 'view'} />
                ...
                <>
                  <div className="space-y-4">
                    <div className="grid grid-cols-[auto_1fr] gap-x-4 gap-y-2 items-center">
                      ...
                      <Select value={repeatFrequency} onValueChange={setRepeatFrequency} disabled={mode === 'view'}>
                      ...
                      <Label className="text-sm">Repeats every:</Label>
                      <div className="flex items-center gap-2">
                        <Select value={repeatInterval.toString()} onValueChange={(val) => setRepeatInterval(parseInt(val))} disabled={mode === 'view'}>
                    ...
                    <div className="space-y-3">
                      ...
                      <RadioGroup value={repeatEndType} onValueChange={(val) => setRepeatEndType(val)} disabled={mode === 'view'}>
                        ...
                        <div className="flex items-center space-x-2 mt-2">
                          ...
                          <div className="flex items-center gap-2">
                            ...
                            <Input type="number" className="w-16" min={1} value={repeatEndCount} onChange={(e) => setRepeatEndCount(parseInt(e.target.value) || 1)} disabled={mode === 'view' || repeatEndType !== 'after'} />
                        ...
                        <div className="flex items-center space-x-2 mt-2">
                          ...
                          <div className="flex items-center gap-2">
                            ...
                            <Input type="date" className="w-auto" value={repeatEndDate ? format(repeatEndDate, 'yyyy-MM-dd') : ''} onChange={(e) => setRepeatEndDate(e.target.value ? new Date(e.target.value) : null)} disabled={mode === 'view' || repeatEndType !== 'on'} />
                          </div>
                        </div>
                      </RadioGroup>
                    </div>
        ...
        {mode !== 'view' && <div className="text-sm text-gray-500 mt-2">* Optional</div>}

        <DialogFooter className="mt-6">
          <Button variant="outline" onClick={onClose}>{mode === 'view' ? 'Close' : 'Cancel'}</Button>
          {(mode === 'create' || mode === 'edit') && (
            <Button variant="default" onClick={handleSave}>{mode === 'edit' ? 'Update' : 'Save'}</Button>
          )}
        </DialogFooter>
      </DialogContent>
    </Dialog>
