...
import { AttachmentPreview, Attachment } from '@/components/messages/AttachmentPreview';
...
const GroupChatPage = () => {
  ...
  const handleOpenDocument = (attachment: Attachment) => {
  ...
  const parseMessageAttachments = (content: string): { text: string; attachments: Attachment[] } => {
    ...
    const mockAttachments: Attachment[] = attachmentLines.map((line, index) => {
      const fileName = line.substring(2); // Remove emoji
      const isImage = line.startsWith('📷');
      
      return {
        id: `mock-${index}`,
        file: new File([], fileName),
        url: isImage ? '/lovable-uploads/127f270b-adf1-49f6-9723-6d800b658c1b.png' : '', // Use actual uploaded image
  ...
  // Enhanced message renderer to show attachments
