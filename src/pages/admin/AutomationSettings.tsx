...
interface Action {
  id: string;
  type: 'award_certificate' | 'send_email' | 'notify_instructor' | 'enroll_course' | 'award_badge' | 'custom';
  title: string;
  description: string;
  dateAdded: string;
...
const AutomationSettings = () => {
  ...
  const [courseCompletionActions, setCourseCompletionActions] = useState<Action[]>([]);
  const [moduleAddedActions, setModuleAddedActions] = useState<Action[]>([]);
  const [moduleCompletionActions, setModuleCompletionActions] = useState<Action[]>([]);
  const [moduleDeletedActions, setModuleDeletedActions] = useState<Action[]>([]);
  const [sectionAddedActions, setSectionAddedActions] = useState<Action[]>([]);
  const [sectionCompletionActions, setSectionCompletionActions] = useState<Action[]>([]);
  const [sectionDeletedActions, setSectionDeletedActions] = useState<Action[]>([]);
  const [assessmentActions, setAssessmentActions] = useState<Action[]>([]);
  const [resourceSharedActions, setResourceSharedActions] = useState<Action[]>([]);
  const [resourceUnsharedActions, setResourceUnsharedActions] = useState<Action[]>([]);
  ...
  const handleEditAction = (action: Action, type: string) => {
  ...
  const handleSaveAction = (actionData: any, type: string) => {
    if (editingAction) {
  ...
  const handleDeleteAction = (actionId: string, type: string) => {
    if (type === 'assessment') {
