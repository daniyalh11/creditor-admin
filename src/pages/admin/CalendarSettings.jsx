...
const CalendarSettings = () => {
  ...
  return (
    <div className="space-y-6 animate-fade-in">
      <PageHeader title="Admin" description="Calendar" />
        ...
        <div className="flex-grow bg-white rounded-lg shadow-sm overflow-hidden border">
          <div className="p-4 border-b flex items-center justify-between">
            ...
            <div className="flex items-center bg-gray-100 rounded-md p-1">
              <Button variant={view === 'week' ? 'default' : 'ghost'} size="sm" onClick={() => setView('week')} className="rounded-md">
                Week
              </Button>
              <Button variant={view === 'month' ? 'default' : 'ghost'} size="sm" onClick={() => setView('month')} className="rounded-md">
                Month
              </Button>
              <Button variant={view === 'agenda' ? 'default' : 'ghost'} size="sm" onClick={() => setView('agenda')} className="rounded-md">
                Agenda
              </Button>
            </div>
          </div>
                  ...
                  <div key={index} className="border-b last:border-b-0">
                    <div className="p-4">
                      <div className="flex gap-2 items-center mb-2">
                        <h3 className={cn('text-lg font-semibold', isCurrentDay && 'text-blue-600')}>
                      ...
                      {agendaItem.events.length === 0 ? (
                        ...
                        <div className="space-y-3">
                          {agendaItem.events.map((event, idx) => (
                            <div key={`${event.id}-${idx}`} className="ml-2">
              ...
              {events.slice(0, 3).map((event, index) => (
                ...
                >
                  ...
                  <div className="text-gray-500">
                    {event.time} - {format(event.days[0], 'MMM d, yyyy')}
                  </div>
                </div>
              ))}
            </div>
